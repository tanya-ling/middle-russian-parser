...
def check_compatibility(sublex, flex, errorHandler=None):
    ...
    are compatible.
    """
    if flex.stemNum is not None and sublex.numStem != flex.stemNum and\
...
class SubLexeme:
    ...
    def __init__(self, numStem, stem, paradigm, gramm, gloss, lex,
        ...
        # (-1 meaning the stem can only be incorporated)
        self.stem = stem
    ...
    def generate_sublexemes(self):
        ...
        if len(self.stemIncorp) > 0:
            ...
            if self.grammIncorp is not None:
                ...
                curGramm = grams[-1][0]
            self.subLexemes.append(SubLexeme(-1, self.stemIncorp, '',
